...
var Page = new Class({
	initialize: function(grid, obj){	
		...
		if(this.url){
			...
			this.iframe = new Element("iframe", {"name": "gridframe", "id": "gridframe", "class":"grid-page-iframe"}).inject(this.body);
			this.iframe.addEvent('load', this.onIframeLoaded.bind(this));
			//this.iframe = frames['gridframe'];
		}
	...
	load:function(){
		...
		// ehm..opera doesn't trigger onload event properly..
		if(!window.opera)this.loader.show();
